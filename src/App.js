...
const PorkageddonGame = () => {
  ...
  const startBattle = () => {
    ...
    // Determine who goes first based on probability
    const randomValue = Math.random();
    if (randomValue < PLAYER_FIRST_TURN_CHANCE) {
      setCurrentTurn('player');
      logMessage("Battle started! You got lucky and go first!");
    } else {
      setCurrentTurn('computer');
      logMessage("Battle started! Casino goes first!");
    }
  };
  ...
  const processPlayerMove = (move) => {
    ...
    setTimeout(() => {
      ...
      // Keep turn as player's since both have now moved
      setCurrentTurn('player');
    }, 1000);
  };
  ...
  const computerTurn = () => {
    ...
    // Computer evaluates its strategy based on health percentages
    const computerHealthPercent = (computerHealth / computerCharacter.health) * 100;
    const playerHealthPercent = (playerHealth / playerCharacter.health) * 100;
    
    let selectedMove;
    
    // Strategy: If health is low and player health is higher, consider healing
    if (computerHealthPercent < 40 && playerHealthPercent > computerHealthPercent && Math.random() < 0.7) {
      selectedMove = computerCharacter.moves.find(move => move.isHealing);
    } 
    // Strategy: If computer is losing badly, small chance to forfeit
    else if (computerHealthPercent < 20 && playerHealthPercent > 60 && Math.random() < 0.1) {
      selectedMove = computerCharacter.moves.find(move => move.isForfeit);
    }
    // Otherwise, select a random attack move
    else {
      const attackMoves = computerCharacter.moves.filter(move => !move.isHealing && !move.isForfeit);
      selectedMove = attackMoves[Math.floor(Math.random() * attackMoves.length)];
    }
    
    if (!selectedMove) {
      // Fallback if we somehow don't have a move
      logMessage("Casino's turn was skipped due to an error.");
      setCurrentTurn('player');
      return;
    }
    
    // Calculate move cost for the selected move
    const moveCost = calculateMoveCost(selectedMove, opponentValue);
    
    // Track casino investment
    setCasinoInvestment(prev => prev + moveCost);
    
    // Add move cost to prize pot
    setPrizePot(prevPot => prevPot + moveCost);
    
    // Handle forfeit
    if (selectedMove.isForfeit) {
      logMessage(`Casino threw in the towel and forfeited the match!`);
      endGame('player', true);
      return;
    }
    
    // Process computer move
    logMessage(`Casino used ${selectedMove.name}! (Cost: $${moveCost} added to pot)`);
    
    if (selectedMove.isHealing && selectedMove.healRange) {
      // Healing move (Penicillin)
      const healAmount = Math.floor(Math.random() * (selectedMove.healRange[1] - selectedMove.healRange[0] + 1)) + selectedMove.healRange[0];
      setComputerHealth(prevHealth => {
        const newHealth = Math.min(computerCharacter.health, prevHealth + healAmount);
        return newHealth;
      });
      logMessage(`Casino healed for ${healAmount} health!`);
    } else if (selectedMove.damageRange) {
      // Attack move (no RTP penalty for Casino)
      const damage = calculateDamage(computerCharacter, selectedMove.damageRange, false);
      setPlayerHealth(prevHealth => {
        const newHealth = Math.max(0, prevHealth - damage);
  ...
  // End game
  const endGame = (winner, forfeit = false) => {
    setGameActive(false);
    setCurrentTurn(null);
  ...
  const addMoney = () => {
    setPlayerMoney(prev => prev + 500);
    logMessage("Added $500 to your account!");
  };
  ...
  }, [playerHealth, computerHealth, gameActive]);

  // Handle computer turn with a delay
  useEffect(() => {
    let timeoutId;
